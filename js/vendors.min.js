...
document.querySelectorAll('.dapp__switch div').forEach(btn => {
	btn.addEventListener('click', () => {
		// Убираем активный класс со всех кнопок
		document.querySelectorAll('.dapp__switch div').forEach(el => el.classList.remove('_active'));
		// Добавляем активный класс текущей кнопке
		btn.classList.add('_active');

		// Получаем значение data-switch
		const target = btn.dataset.switch;

		// Скрываем оба блока
		document.querySelector('.dapp__website').classList.remove('_active');
		document.querySelector('.dapp__design').classList.remove('_active');

		// Показываем нужный
		document.querySelector(`.dapp__${target}`).classList.add('_active');
	});
});
